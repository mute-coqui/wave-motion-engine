use tobj::{load_obj, LoadError, Material, Model};
...
impl Scene {
    pub fn new(path: &str) -> Result<Self, LoadError> {
        let (models, materials) =
            load_obj(&path, &tobj::LoadOptions::default()).expect("Failed to OBJ load file");
        Ok(Scene {
            models,
            materials: materials?,
        })
...
//         println!("    material.Ka = ({:?})", m.ambient);
//         println!("    material.Kd = ({:?})", m.diffuse);
//         println!("    material.Ks = ({:?})", m.specular);
